...
import {
  fetchDoctorProfile,
  updateDoctorProfile,
} from "./Slices/doctorProfileSlice"; // তোমার সঠিক পাথ মিলিয়ে নিও

// ওয়ার্কিং ডেজ এর একটি কনস্ট্যান্ট অ্যারে ম্যাপ করার জন্য
const DAYS_OF_WEEK = [
  "Sunday",
  "Monday",
  "Tuesday",
...
function DoctorProfile() {
  ...
  // ১. স্টেট সেপারেশন (অ্যাডভান্সড মেমরি ম্যানেজমেন্ট)
  ...
  // ২. দ্য ইনিশিয়ালাইজার (ডেটা ফেচ করা)
  useEffect(() => {
  ...
  // ৩. দ্য হাইড্রেটর (ডেটা ফর্মে বসানো)
  useEffect(() => {
    if (profile) {
      setFormData({
  ...
  const handleCancel = () => {
    setIsEditing(false);
    // বাতিল করলে আগের ডেটায় রিভার্ট করে দাও
    if (profile) {
      setFormData({
        consultationFee: profile.consultationFee || "",
        experienceYears: profile.experienceYears || "",
  ...
  const handleWorkingDayToggle = (day) => {
    setWorkingDays((prev) =>
      prev.includes(day)
        ? prev.filter((d) => d !== day) // Update: বাগ ফিক্স করা হয়েছে (index এর বদলে day দিয়ে চেক)
        : [...prev, day],
    );
  };

  // ৫. দ্য এক্সিকিউশন ইঞ্জিন
  const handleSubmit = async (e) => {
    ...
    try {
      // .unwrap() দিয়ে অ্যাসিনক্রোনাস থাঙ্ক এর রিয়েল রেজাল্ট ক্যাচ করা হচ্ছে
