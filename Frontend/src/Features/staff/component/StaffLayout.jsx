...
import { LayoutDashboard, CalendarClock, FileText, Activity, Menu, X, ArrowLeft } from "lucide-react";
...
const StaffLayout = () => {
  ...
  const isDashboard = location.pathname === "/staff/dashboard" || location.pathname === "/staff";
  ...
  return (
    <div className="flex h-screen w-full bg-[#03090a] text-slate-200 overflow-hidden font-sans selection:bg-teal-500/30">
      ...
      >
        <div className="h-20 flex items-center justify-between px-6 border-b border-white/[0.05]">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-slate-200 tracking-tight">StaffOS</h1>
          </div>
          <button 
            onClick={closeMobileMenu}
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative z-10 w-full">
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar relative p-4 md:p-8 bg-[#03090a]">
          <Outlet />
        </main>
      </div>
    </div>
